...
from WASMaker.fuzzer.mutation import primitive2simd
...
def get_avaliable_ast():
    ...
    for i in range(len(collist)):
        ...
        avaliable_ast.append((i, AST_count))

    myclient.close()
    return avaliable_ast
...
def get_valid_instrs():
    ...
    for i in range(len(collist)):
        ...
        if AST_count != 0:
            ...
            valid_instrs.append(all_AST_json)
...
def emit_funcbody(AST_num, module, valid_instrs, is_simd, functype=None):
    local_list = []
    ...
    for i in range(AST_num):
        ...
        instr_index = random.randint(0, len(valid_instrs) - 1)

        instr_collection = valid_instrs[instr_index]
        ...
        if functype != None:
            emit_node_context(module, local_list, random_AST, valid_instrs, is_simd, functype)
        else:
            emit_node_context(module, local_list, random_AST, valid_instrs, is_simd)
...
def emit_node_context(module, local_list, node, valid_instrs, is_simd, functype=None):
    node_stack = []
        ...
        if context is not None:
            ...
            if context.local_variable:
                ...
                if stack_depth >= 16:
                    ...
                    funcbody, func_local_list, results_type = emit_funcbody(random.randint(1, 10), module, valid_instrs,
...
def emit_wasm_bianry(file_name, valid_instrs, is_simd):
    module = Module(1836278016, 1)

    funcbody, locals_list, results_type = emit_funcbody(10, module, valid_instrs, is_simd)

    functype = FuncType(FtTag, [], results_type)
    functype_id = None
    for _, type_item in enumerate(module.type_sec):
        if type_item == functype:
            functype_id = _
    if functype_id is None:
